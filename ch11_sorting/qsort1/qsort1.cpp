...
void qsort1(int *array_data, int array_size, int low, int up)
{
	int i, temp;
	int mid, mid_val;
	...
	qsort1(array_data, array_size, low, mid-1);
	qsort1(array_data, array_size, mid+1, up);
...
int main()
{
	...
	printf("Sorting data...\n");
	start = clock();
	qsort1(array_data, ARRAY_SIZE, 0, ARRAY_SIZE-1);
	finish = clock();
